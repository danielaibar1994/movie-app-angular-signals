...
export class MoviesSignal {
  ...
  setMovieDetail(movie: Movie, actors: Actor[], companies: Company[]): void {
    ...
    const companyName =
      (movie.company &&
        companies.find(
          (company) => company.id.toString() === movie.company?.toString()
        )?.name) ||
      '';
  ...
  private callMoviesList(): void {
    this.repository
      ...
      .pipe(
        retry(2),
        first(),
        tap((moviesData) => moviesSignal.set(moviesData))
      )
      .subscribe();
  }
  ...
  private callMovieDetail(id: string): void {
    combineLatest([
      ...
      .pipe(
        tap(([movie, actors, companies]) =>
          this.setMovieDetail(movie, actors, companies)
        )
      )
      .subscribe();
  }
  ...
  private callActorsList(): void {
    this.actorsRepository
      ...
      .pipe(
        retry(2),
        first(),
        tap((actors) => actorsSignal.set(actors))
      )
      .subscribe();
  }
  ...
  private callCompaniesList(): void {
    this.companiesRepository
      ...
      .pipe(
        retry(2),
        first(),
        tap((companies) => companiesSignal.set(companies))
      )
      .subscribe();
  }
  ...
  private callEditMovie(id: number, movie: Movie): void {
    this.repository
      .updateItem(id.toString(), movie)
      .pipe(
        retry(2),
        first(),
        tap((editedMovie) => {
          this.setMovieDetail(editedMovie, actorsSignal(), companiesSignal());
        })
      )
      .subscribe();
  }
